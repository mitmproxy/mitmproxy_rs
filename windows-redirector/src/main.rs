...
async fn main() -> Result<()> {
    ...
    let mut listen_sockets = HashMap::<(SocketAddr, TransportProtocol), ProcessInfo>::new();
                    ...
                    None => {
                        if address.outbound() {
                            ...
                            let action = {
                                ...
                                if !listen_sockets.contains_key(&(dst, proto)) {
                                    dst.set_ip(IpAddr::V4(Ipv4Addr::UNSPECIFIED));
                                }
                                if let Some(proc_info) = listen_sockets.get(&(dst, proto)) {
                                    debug!(
            ...
            Event::SocketInfo(address) => {
                ...
                match address.event() {
                    ...
                    WinDivertEvent::SocketListen => {
                        let mut listen_sock = connection_id.src;
                        if listen_sock.ip() == IpAddr::V6(Ipv6Addr::UNSPECIFIED) {
                            // Dual-stack binds: binding to [::] actually binds to 0.0.0.0 as well.
                            listen_sock.set_ip(IpAddr::V4(Ipv4Addr::UNSPECIFIED))
                        }
                        let pid = address.process_id();
                        let process_name = get_process_name(pid).ok();
                        debug!("Registering {:?} on {}.", process_name, listen_sock);
                        listen_sockets.insert(
                            (listen_sock, proto),
                            ProcessInfo {
                                pid,
                                process_name,
                    ...
                    WinDivertEvent::SocketClose => {
                        ...
                        listen_sockets.remove(&(listen_sock, proto));
                    }
                    _ => {}
                }
            }
        ...
        match packets {
            Ok(packets) => {
                for packet in packets {
                    if let Err(_) = tx.send(Event::SocketInfo(packet.address)) {
    ...
    loop {
        ...
        match packets {
            Ok(packets) => {
                for packet in packets {
                    if let Err(_) =
                        tx.send(Event::NetworkPacket(packet.address, packet.data.into()))
                    {
                        return; // main thread shut down.
                    }
                }
...
async fn insert_into_connections(
    ...
    let existing1 = connections.insert(connection_id.reverse(), ConnectionState::Known(ConnectionAction::None));
...
async fn process_packet(
    ...
    match action {
        ConnectionAction::None => {
            debug!(
                "Re-injecting {} {} outbound={} loopback={}",
                packet.connection_id(),
                packet.tcp_flag_str(),
                address.outbound(),
                address.loopback()
